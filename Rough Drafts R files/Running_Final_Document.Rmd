...
colnames(M) <- c("Age Alcohol", "Age Marijuana", "Sexual Partners", "Physical Activity", "Sleep (hrs)","Age Romantic")
rownames(M) <- c("Age Alcohol", "Age Marijuana", "Sexual Partners", "Physical Activity", "Sleep (hrs)","Age Romantic")
corrplot(M, addCoef.col = 1, number.cex = .5,method = c("color"),col = COL2('PRGn'),addgrid.col = 'white',tl.col='black',tl.cex=1)

```

We know that a correlation matrix depicts the correlation coefficients between all the possible pairs of values in a table. Positive correlations are displayed in green and negative correlations are in purple. Color intensity is proportional to the correlation coefficients. We see a mix of positive and negative correlations in the matrix. We see a high positive correlation between the age when the respondent had their first alcoholic drink with age when the respondent first started smoking marijuana and number of hours the respondent sleeps in a day. Also, there is a positive correlation between age when the respondent first started smoking marijuana with number of hours the respondent sleeps in a day and the age when the respondent had their first romantic experience. We see significant negative correlations of number of sexual partners the respondent has with age when the respondent had their first alcoholic drink, age when the respondent first smoked marijuana, number of hours the respondent sleeps in a day, and age when the respondent had their first romantic experience.  
...
```{r}
varImpPlot(model1c, bg = "purple", cex=1, pch=22, main="RF Feature Importance")
```
...
```{r}
plot(model2c)
varImpPlot(model2c, bg = "purple", cex=1, pch=22, main="RF Feature Importance")
```
...
```
```{r}
varImpPlot(model3c, bg = "purple", cex=1, pch=22, main="RF Feature Importance")
```
...
```{r}
varImpPlot(model4c, bg = "purple", cex=1, pch=22, main="RF Feature Importance")
```
...
```{r}
varImpPlot(model5c, bg = "purple", cex=1, pch=22, main="RF Feature Importance")
```
